...
> 当前主程序：**小六壬**。这是为 FoloToy AI Passport（ESP32-C3、240 × 320 屏幕、三枚实体键和麦克风）制作的离线小六壬应用。

## 小六壬应用

- `OK` 长按时默念所问，松开后依次录入三次直觉数字；
- `UP` / `DOWN` 选取 1–9，按住后立即连续翻页并逐渐加速；
- 结果提供四页可上下翻阅的中文解析；
- 全程离线：麦克风只用于本地音量检测，不保存或上传录音；
- 视觉使用文渊宋体、全屏宣纸纹理与墨印标题效果。

构建前请安装 ESP-IDF 5.5.3，并在项目目录执行：
